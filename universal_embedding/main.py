...
def get_dataset(dataset_name: str, transform: Callable, transform_train=None) -> Dict:
    ...
    root = "/mnt/bn/yangmin-priv-fashionmm/database/products"
    ...
    if dataset_name == "aliproduct":
        ...
        # print(dataset_name, trainset.num_classes)
        ...
        testset = MySOP(root, "eval", "Ebay_train_concat3.txt", transform)

        trainset = torch.utils.data.ConcatDataset([trainset_aliproduct, trainset_products10k, trainset_sop])
        print(len(trainset))
        trainset.num_classes = 71039
        print("final dataset has {} classes".format(trainset.num_classes))
        return {"train": trainset, "test": testset, "metric": "rank1"}
...
def get_transform(image_size: int = 224, is_train: bool = True):
    ...
    t.append(transforms.Resize(224, interpolation=PIL.Image.BICUBIC))
