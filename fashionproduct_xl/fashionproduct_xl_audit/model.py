...
from easyguard.modelzoo.models.falbert.albert import Transformer
...
class FPXL(CruiseModule):
    ...
    def setup(self, stage) -> None:
        ...
        self.cls_encoder = Transformer(cls_config)
        ...
        if self.hparams.use_multihead:
            ...
            self.cls_layer = torch.nn.Linear(self.hparams.hidden_dim, self.hparams.class_num)
        self.tau = torch.nn.Parameter(torch.tensor([np.log(1.0 / 0.07)], dtype=torch.float32))
    ...
    def forward_step(
        ...
        hidden_state = rep_dict["hidden_states"]
        concat_mask = torch.cat(
            [input_mask, torch.ones(input_mask.shape[0], 1, device=input_mask.device), frames_mask], dim=1
        )
        hidden_concat = torch.cat(hidden_state, dim=-1)  # batch, length, feature*N
        hidden_max_concat = self.maxpooling_with_mask(hidden_concat, attention_mask=concat_mask)
        hidden_mean_concat = self.meanpooling_with_mask(hidden_concat, attention_mask=concat_mask)

        seq_feat = torch.cat(
            [
                cls_emb.unsqueeze(1),
                hidden_max_concat.reshape(batchsize, -1, self.hparams.hidden_dim),
                hidden_mean_concat.reshape(batchsize, -1, self.hparams.hidden_dim),
            ],
            dim=1,
        )

        # cls_feat = self.cls_encoder(seq_feat)
        all_hidden, att_prob = self.cls_encoder(
            seq_feat, mask=torch.ones(seq_feat.shape[0], seq_feat.shape[1], device=seq_feat.device)
        )
        cls_feat = all_hidden[-1]
        feat_pooling = cls_feat.mean(dim=1)
        if self.hparams.use_multihead:
            logits = self.multi_heads_with_mask(feat_pooling, head_mask)
        else:
            logits = self.cls_layer(feat_pooling)

        self.tau.data = torch.clamp(self.tau.data, 0, 4.6051)
    ...
    def training_step(self, batch, idx):
        ...
        res = {"loss": loss, "train_lr": self.trainer.lr_scheduler_configs[0].scheduler.get_last_lr()[0]}
