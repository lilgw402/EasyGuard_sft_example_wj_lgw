...
from valley.constants import WORKER_HEART_BEAT_INTERVAL
...
from valley.utils import (build_logger, server_error_msg,
    pretty_print_semaphore)
from valley.util.data_util import load_video
from valley.model.modelling_valley import ValleyLlamaForCausalLM
...
def load_model(model_path, model_name, num_gpus):
    ...
    if 'lora' in model_path:
        ...
        if 'config.json' in os.listdir(model_path):
            model_old = ValleyLlamaForCausalLM.from_pretrained(model_path)
        else:
            model_old = ValleyLlamaForCausalLM.from_pretrained(config.base_model_name_or_path)
        print('load lora model')
        model = PeftModel.from_pretrained(model_old, model_path)
        ...
        print('load model')
        if 'checkpoint-' in model_path : # if load a checkpoint, then use the tokenizer in parent folder
            tokenizer_path = '/'.join(model_path.split('/')[:-1])
            if not Path(tokenizer_path).is_file():
                tokenizer_path = model_path
            # tokenizer_path = '../../checkpoints/stable-valley-13b-v1'
        else:
            tokenizer_path = model_path
        ...
        while True:
            try:
                ret = requests.post(url, json={
                    "worker_name": self.worker_addr,
                    "queue_length": self.get_queue_length()}, timeout=10)
    ...
    def generate_video_stream(self, params):
        ...
        if videos is not None and len(videos) > 0 and self.is_multimodal:
            ...
            if len(videos) > 0:
                ...
                # videos = [load_video(video, frame_mode='fps',fps_number=0.4, fixed_frame_number = params['fixed_frame_number']) for video in videos]
                videos = [load_video(video, fixed_frame_number = params['fixed_frame_number']) for video in videos]
