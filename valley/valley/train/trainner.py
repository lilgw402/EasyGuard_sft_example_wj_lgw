...
import datasets
from typing import Callable, Dict, List, Optional, Tuple, Union, Iterator, Iterable, Sized, Generic, TypeVar
...
class ValleyTrainer(Seq2SeqTrainer): #实例化时所需要的参数，并将这些参数传递给其父类 `Seq2SeqTrainer` 的初始化函数
    def __init__(self,
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        '''
        inputs: 
        {'input_ids': tensor([[    1,   31...='cuda:1'),  batch_size x sequence length
        'labels': tensor([[ -100,  -10...='cuda:1'), batch_size x sequence length
        'attention_mask': tensor([[ True,  Tru...='cuda:1'), batch_size x sequence length
        'images': [ tensor([[[[ 0.0471, ...h.float16), 
                    tensor([[[[-0.0696, ...h.float16), 
                    tensor([[[[ 1.7256, ...h.float16), 
                    tensor([[[[-1.7197, ...h.float16), 
                    tensor([[[[1.7256, 1...h.float16), 
                    tensor([[[[0.7627, 0...h.float16), 
                    tensor([[[[ 1.9307, ...h.float16), 
                    tensor([[[[-0.3762, ...h.float16), 
                    tensor([[[[-1.5879e+...h.float16), ...]} batch_size
