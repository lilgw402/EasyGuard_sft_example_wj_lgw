...
class EndpointHandler:
    def __init__(self):
        ...
        self.model_path = "llama_7b_ccr/"  # 需要加载的模型放在当前根目录的llama_7b_ccr文件夹下
        ...
        self.model = AutoModelForCausalLM.from_pretrained(
            ...
            torch_dtype=torch.float16,  # 默认使用float16加载,减少显存占用
            device_map="auto",  # 需要安装huggingface的accelerate
        )
    ...
    def __call__(
        ...
        # 注意输入和返回值必须要满足这样的格式, Laplace的要求，否则会部署失败
        start = time.time()
...
if __name__ == "__main__":
    ...
    # 这里为了对比速度，bs设置了显存可接受的最大值。但是对输出结果有影响，原因暂时还没找到
    # bs=1的时候输出是正常的
    bs = 64
    ...
    with open("hf_bs64_max_length.json", "w", encoding="utf-8") as json_file:
