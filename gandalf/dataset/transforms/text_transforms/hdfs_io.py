    ...
    for folder in folders:
        if folder.startswith("hdfs"):
            ...
            if os.path.isdir(folder):
                ...
                logging.info("Path {} is invalid".format(folder))
...
def fetch_file_from_hdfs(file_path, dir_path="/tmp"):
    ...
    if file_path.startswith("hdfs"):
        ...
        local_file_path = os.path.join(
            dir_path, f"{file_path.split('/')[-1]}_{random_string(10)}_{rank}"
        )  # 按 file_name, random_string, rank 作为名字
        logging.info(f"{file_path} is on hdfs, copy it to {local_file_path}")
