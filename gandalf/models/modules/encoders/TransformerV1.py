...
class TransformerEncoderv2(nn.Module):
    ...
    def __init__(
        ...
        self._return_attn_score = return_attn_score
    ...
    def forward(self, x_in, x_in_k=None, x_in_v=None):
        ...
        for layer in self.layers:
            if x_in_k is not None and x_in_v is not None:
                if self._return_attn_score:
                    x, attn_scores = layer(x, x_k, x_v)
                else:
                    x = layer(x, x_k, x_v)
            else:
                if self._return_attn_score:
        ...
        if self._return_attn_score:
            return x, torch.stack(attn_score_list, dim=0)
        return x
